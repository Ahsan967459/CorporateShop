...
const SignUp = () => {
    ...
    const {createUser,setUser}=useContext(context);
    ...
    const getUserEmailPassw=(e)=>{
          ...
          if(password.length<8){
            setUserCreateError("Give password 8 digit or longer");
            return
          }else if(!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(password)){
            setUserCreateError("Use special character ($,&,*,A-Z,a-z,Number)");
            return
          }else if(!checkTC){
            setTermsAccept("please accept our terms and condition")
              return
          ...
          .then(res=>{
              ...
              updateProfile(res.user,{ displayName:name, photoURL: "https://example.com/jane-q-user/profile.jpg"})
                .then(res=>{
                  setUserCreateSuccess(res.user);
                  setUser(null);
                })
    ...
    return (
        ...
        style={{ backgroundImage: `url('https://i.ibb.co/FnmYwMX/MOS-Shopfront-202112.jpg')`,backgroundRepeat:"no-repeat", backgroundPosition:"center",backgroundSize:"cover" }}
        >
       ...
       {userCreateSuccess && <div className="w-full h-screen absolute bg-gray-600/70 z-10 flex justify-center items-center">
            ...
            <div> <p className="text-[20px] text-green-700 font-semibold">Account Creation Successfully.</p><p className="text-[20px] text-red-700 font-semibold">Please,login correct email and<br></br> password.</p></div>
            ...
            <div className="w-fit bg-green-400 p-8 py-12  rounded-xl">
                <form onSubmit={getUserEmailPassw} >
                    ...
                    <input className="mb-5  w-[290px] sm:w-[360px] bg-white text-black  py-[3px] px-4 rounded" type="text" name="name" key="1" placeholder="User Name" required />
                    <br />
                    <input className="mb-5 bg-white text-black  w-[290px] sm:w-[360px] py-[3px] px-4 rounded" type="email" name="email" key="2" placeholder="User Email" required/>
                    <br />
                    <div className="flex relative">
                        <input key="2" className="mb-5  w-[290px] sm:w-[360px] py-[3px] px-4 bg-white text-black rounded" type={showPassword ? "text":"password" } name="password" id="" placeholder="Password" required />
                    ...
                    <input className="mb-5 mt-2  w-[290px] sm:w-[360px] py-[5px] px-4 rounded bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 font-semibold text-white cursor-pointer" type="submit" value="Sign Up" required/>
               </form>
